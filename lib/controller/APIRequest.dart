...
class API_Request {
  ...
  static Future<Map<String,dynamic>> api_upload_query(String filepath, String filename, String uploadfoldername) async {
    String url = '';
    url = await LocalDataAccess.getVariable('serverIP');
    if (url == '' || url == 'MAIN_SERVER') {
    ...
    String savedToken = await LocalDataAccess.getVariable("token");    
     FormData body = FormData.fromMap({
        'uploadedfile': await MultipartFile.fromFile(filepath, filename: filename),
        'filename': filename,
    ...
    try {
      final response = await dio.post(url, data: jsonEncode(body2),  onSendProgress: (int sent, int total) {
